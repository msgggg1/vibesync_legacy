...
function showCombinedSettingsView() {
    ...
    if (isKakaoLinked) {
        $('#kakaoLinkContainer').html('<button id="btnKakaoUnlink" class="modal-btn-kakao-unlink"><img id="kakao_img" src="${ctx}/resources/images/icons/KakaoTalk_logo.svg" alt="카카오 로그인"><span>카카오 연결 해제</span></button>');
    } else {
        $('#kakaoLinkContainer').html('<a style="font-size:12px;" href="${ctx}/auth/kakao/link" class="modal-btn-kakao-link"><img id="kakao_img" src="${ctx}/resources/images/icons/KakaoTalk_logo.svg" alt="카카오 로그인"><span>카카오 계정 연결</span></a>');
    }
}
            ...
            success: function(response) {
                if (response.posts && response.posts.length > 0) {
                    ...
                    $.each(response.posts, function(index, post) {
                        var thumbnailUrl = post.thumbnail_img ? contextPath + post.thumbnail_img : contextPath + 'vibesync/resources/images/system/default_thumbnail.png';
