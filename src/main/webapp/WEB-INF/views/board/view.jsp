...
<div id="postview_Wrapper">
	<div class="title">
		<p>${noteDetail.note.title}</p>
		<c:if test="${sessionScope.memberInfo != null && sessionScope.memberInfo.acIdx == noteDetail.upacIdx}">
			<div>
				<button class="postview_ed_btn">
					<a href="edit?noteIdx=${noteDetail.note.noteIdx}">edit</a>
				</button>
				<button class="postview_de_btn">
					<a href="delete?noteIdx=${noteDetail.note.noteIdx}" onclick="return confirm('정말로 삭제하시겠습니까?');">delete</a>
				</button>
			</div>
		</c:if>
	</div>
	<div class="writer_info">
		<div class="writer">
			<img
				src="${(noteDetail.note.img == null or empty noteDetail.note.img) ? '${pageContext.request.contextPath}/sources/default/default_user.jpg' : note.img}"
				alt="writer_profile"> <a
				href="userPage.do?acIdx=${noteDetail.member.acIdx}">${noteDetail.member.nickname}</a>
			<c:if
				test="${not empty sessionScope.memberInfo and sessionScope.memberInfo.acIdx != noteDetail.upacIdx}">
				<form id="followForm"
					style="display: inline; margin: 0; padding: 0;">
					<button id="followBtn" type="submit" data-user-idx="${memberInfo.acIdx}"
						data-writer-idx="${noteDetail.upacIdx}" data-nidx="${noteDetail.note.noteIdx}"
		...
		<div class="like_share">
			<div>
				<p>
					<span>view : </span><span>${noteDetail.note.viewCount}</span>
				</p>
			</div>
			<form id="likeForm" style="display: inline; margin: 0; padding: 0;">
				<button id="likeBtn" type="submit" data-user-idx="${memberInfo.acIdx}"
					data-note-idx="${noteDetail.note.noteIdx}"
					...
					<img id="likeImg"
						...
						id="likeCount" style="vertical-align: middle;">${noteDetail.likeNum}</span>
				</button>
			</form>
		</div>
	</div>
	...
	<div class="text_content">
		<c:out value="${noteDetail.note.text}" escapeXml="false" />
	</div>
		...
		<c:choose>
			...
			<c:when test="${not empty memberInfo}">
				<form id="comment-form" style="margin-bottom: 1.864rem;">
					<input type="hidden" name="noteIdx" value="${noteDetail.note.noteIdx}">
...
const isLoggedIn = ${not empty memberInfo}; 
...
function requireLogin() {
   ...
   const ctx = "${pageContext.request.contextPath}";
    ...
    $(document).ready(function() {
      ...
      const ajaxUrl = '${pageContext.request.contextPath}/vibesync/view';
      const ctx = '${pageContext.request.contextPath}';
      ...
      // =================== 대댓글 기능이 포함된 스크립트 ===================
      const commentUrl = '${pageContext.request.contextPath}/vibesync/comment.do';
      const loggedInUserIdx = ${not empty memberInfo ? memberInfo.acIdx : -1};
      const noteIdxForComment = ${noteDetail.note.noteIdx};
