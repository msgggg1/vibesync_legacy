...
function loadWatchPartyList() {
    fetch(`${CONTEXT_PATH}/watch/list`)  // 수정: 기존 "/WatchPartyListServlet" → "/watch/list"
        .then(response => {
            if (!response.ok) throw new Error('서버 응답 실패: ' + response.status);
            return response.json();
        })
...
function renderList(data, containerId) {
    ...
    data.forEach(item => {
        ...
        `;
        const wpIdx = item.watchParty_idx || item.watchPartyIdx;
        li.dataset.watchPartyIdx = wpIdx;
        li.addEventListener('click', () => {
            window.location.href = `${CONTEXT_PATH}/watch/${wpIdx}`;  // 수정: JSP 경로 → Spring MVC URL
        });
        ul.appendChild(li);
    });
...
document.getElementById('add-form').addEventListener('submit', function(e) {
    ...
    // 4) AJAX 요청으로 서버에 전송
    fetch(`${CONTEXT_PATH}/watchParty/add`, {  // 수정: "/AddWatchPartyServlet" → "/watchParty/add"
        method: 'POST',
...
function connectSyncWebSocketForHost() {
    wsSyncHost = new WebSocket("ws://" + location.host + CONTEXT_PATH + "/waSyncEndpoint");  // unchanged
    wsSyncHost.onopen = () => {
